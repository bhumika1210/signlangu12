...
// Function to send the detected emotion to the chatbot
async function sendEmotionToChatbot(emotion) {
  try {
    // Make an API call or WebSocket connection to send the emotion to the chatbot server
    ...
    // Process the chatbot response as needed
  } catch (error) {
    console.error('Error sending emotion to chatbot:', error);
  }
}

// Inside the predict() function after getting the predicted emotion
const predictedEmotion = this.currentPredictedWords[0];
this.predictionOutput.showOutput(predictedEmotion);

// Send the detected emotion to the chatbot
sendEmotionToChatbot(predictedEmotion);

// Function to navigate to the chatbot file
function goToChatbot() {
  // Replace 'chatbot.html' with the filename or URL of your chatbot file
  window.location.href = 'templates/index.html';
}

// Inside the predict() function after sending the detected emotion to the chatbot
sendEmotionToChatbot(predictedEmotion);

// Navigate to the chatbot file
goToChatbot();
